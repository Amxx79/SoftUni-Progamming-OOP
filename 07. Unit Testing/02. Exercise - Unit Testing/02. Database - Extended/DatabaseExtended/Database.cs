...
namespace ExtendedDatabase
{
    public class Database
    {
        ...
        public Database(params Person[] persons) //Ready
        {
            this.persons = new Person[16];
            AddRange(persons);
        }

        public int Count //Ready
        {
            get { return count; }
        }

        private void AddRange(Person[] data) //Ready
        {
            if (data.Length > 16)
            {
        ...
        public void Add(Person person) //Ready
        {
            if (this.count == 16)
            {
        ...
        public Person FindByUsername(string name) //Ready
        {
        ...
        public Person FindById(long id) //Ready
        {
            if (id < 0)
            {
